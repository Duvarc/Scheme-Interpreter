...
def make_call_frame(procedure, args, env):
    ...
    # BEGIN Question 12
    if type(procedure) is LambdaProcedure:
...
def do_if_form(expressions, env):
    ...
    elif expressions.second.second:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_true(test):
            # BEGIN Question 15A
            """
            val = eval_all(scheme_eval(expressions.first.second, env), env)
            val = scheme_eval(eval_all(expressions.first.second, env), env)
            elif self_evaluating(val):
                return val
            """ 
            val = expressions.first.second
            if val is nil:
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    """
    def construct_formals(lst):
        while lst is not nil:
            formals.first = lst.first.first
            lst = lst.second
            formals.second = construct_formals(lst)

    def construct_values(lst):
        while lst is not nil:
            values.first = values.first.second
            lst = lst.second
            values.second = construct_values(lst)

    formals = construct_formals(bindings)
    values = construct_values(bindings2)
    """

    check_form(formals, 1, len(values))
...
def check_formals(formals):
    ...
    while formals:
        ...
        formals = formals.second
    if len(set(formals_list)) != len(formals_list):
        raise SchemeError
