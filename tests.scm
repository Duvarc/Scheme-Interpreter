...
(define x (make-adder 4))
; expect x

(define y (make-adder 456))
; expect x
...
(define (sub-all s olds news)
  (cond
    ((null? olds) s)
    (else
      (define new_s (substitute s (car olds) (car news)))
      (sub-all new_s (cdr olds) (cdr news))
    )
  )

(define lst '(2 2 2 3 8 2 8))
; expect lst

(sub-all lst 2 4)
; expect (4 4 4 3 8 4 8)
...
(define (compose f g x)
  (f (g x)))
(compose square divide-by-5 25)
;expect 25

(compose divide-by-5 square 10)
;expect 20
...
(define (abs-x-minus-abs-y x y)
  (- (abs x) (abs y)))

(abs-x-minus-abs-y -3 -5)
;expect -2
...
(square (square (fourth-root 128)))
; expect 128.00000315926354
...
((lambda (a b c) (+ (+ (square a) (square b)) (square c))) 1 2 3)
; expect 14
